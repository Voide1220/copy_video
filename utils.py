...
def load_frames_opencv(video_dir, start=0, end=None, crop=None, resize=None):
    ...
    frames = []
    frames_names = [video_dir+v for v in os.listdir(video_dir)]

    for frame_id in range(len(frames_names)):
        # frame_file = os.path.join(video_dir, f'{frame_id:04d}.png')
        frame_file = frames_names[frame_id]
...
def load_frames(video_id='', video_dir=None, start=0, end=None, window=None, repeat_times=1, resize=256, crop=None):
    ...
    if repeat_times > 1:
        ...
        if window is not None and start == 0 and end is None:
            video_len = get_video_length(video_id)  ## 直接已经是帧了
            if video_len > window:
                start = np.random.randint(max(video_len - window, 1))
                end = start + window
