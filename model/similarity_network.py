...
class SimilarityNetwork(enum.Enum):
    ...
    def _get_config(self, value, **kwargs):
        return {self.ViSiL: ViSiL(**kwargs)}[value]
...
class ViSiL(SimilarityNetworkABC):
    ...
    def __init__(self,
        ...
        self.visil_head = VideoComparator(batch_norm=batch_norm)   ## 额外多出来的卷积网络
        ...
        if pretrained is not None:
            self.idx_type = 'att'
            self.attention = Attention(dims, norm=attention_norm, activation=attention_activation)
            self.load_state_dict(torch.hub.load_state_dict_from_url(model_urls[pretrained])['model'])
    ...
    def similarity_matrix(self, query, target, batched=False,
        ...
        sim = self.activation(sim)        ## 激活还是要变成0-1之间
    ...
    def forward(self, x, masks=None):
        ...
        sim, sim_mask = self.frame_to_frame_similarity(x, x, masks, masks, batched=False)
        ...
        return sim.view(N, N), loss  ## 正则化loss
