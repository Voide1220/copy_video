...
def main(args):
    ...
    nce_criterion = InfoNCELoss(args.temperature)   ## 对比学习常用的infonce_loss
    ...
    for epoch in range(global_step // len(loader), args.epochs):
        ...
        meters.reset()
        model.train()
        global_step = train_one_epoch(epoch, global_step, feat_extractor, model, loader, optimizer, lr_scheduler, fp16_scaler,
...
def train_one_epoch(epoch, global_step, extractor, model, loader, optimizer, lr_scheduler, fp16_scaler,
    ...
    for idx, (videos, labels) in enumerate(pbar):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            similarities, regularization_loss = model(features)   ## 正则化loss有啥用？
        ...
        meters.update('reg_loss', regularization_loss)  ## 把算出来的相似度矩阵，进行[1,-1]的归一化，然后再算l1 norm
