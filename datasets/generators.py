...
class SSLGenerator(TrainDataset):
    ...
    def __init__(self, dataset_path, weak_aug=None, strong_aug=None, window_sz=32, percentage=1., **kargs):
        ...
        self.dataset_path = dataset_path.split(',')   ## 视频地址
        self.videos = []

        for vdir in self.dataset_path:
            # self.videos += [v.replace('/00000.jpg', '') for v in glob.glob(os.path.join(vdir, '*/00000.jpg'))]
            self.videos = [vdir+v for v in os.listdir(vdir)]

        self.videos = sorted(self.videos)
        self.videos = self.videos[:int(len(self.videos)*percentage)]  ## 取出全部视频
        # ipdb.set_trace()

    def next_epoch(self):
        random.shuffle(self.videos)   ## 直接处理的视频
    ...
    def __getitem__(self, idx):
        ...
        video = utils.load_frames(video_dir=video_dir, window=self.window_sz*2)   ## 每个视频抽帧
